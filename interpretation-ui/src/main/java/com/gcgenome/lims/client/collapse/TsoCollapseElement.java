...
public class TsoCollapseElement extends HTMLElementBuilder<HTMLDivElement, TsoCollapseElement> implements CollapseElement<HTMLDivElement> {
    public static TsoCollapseElement build(String id, long sample, JsPropertyMap<?> service) {
        return new TsoCollapseElement(div(), id, sample, service);
    }
    ...
    private TsoCollapseElement(HTMLContainerBuilder<HTMLDivElement> e, String id, long sample, JsPropertyMap<?> service) {
    ...
    public void update() {
        ...
        Promise<InterpretationTso> interpretation = InterpretationApi.interpretation(sample, (String)service.get("code")).then(Js::cast);
        interpretation.then(this::update).catch_(error->update(null));
    }
    private Promise<Void> update(InterpretationTso map) {
        meta.element().innerHTML = "";
        summaryTier1.element().innerHTML = "";
        summaryTier2.element().innerHTML = "";
        summaryTier3.element().innerHTML = "";
        if(map==null || map.variants==null) {
    ...
    /*private static void update(InterpretationTso.Result result, HTMLContainerBuilder<HTMLLabelElement> label) {
    ...
    @Override
    public TsoCollapseElement that() {
        return this;
    }
