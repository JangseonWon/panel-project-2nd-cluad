...
import com.gcgenome.lims.testcontainer.MessageQueue
...
internal class IntegrationTest(
    ...
    private val queue: Sinks.Many<PanelEvent<*, *>>
): BehaviorSpec({
    Given("큐에 메시지가 발행 중") {
        val user = "user"
        db.insertUserAsAdmin(user)
        val client = _client.mutate().defaultHeaders { headers ->
            headers.add("X-USER-ID", user)
        }.build()
        Flux.interval(Duration.ofMillis(10)).subscribe {
            queue.tryEmitNext(TestEvent(UUID.randomUUID(), "Test$it", PanelEvent.Type.entries.random()))
        }
    ...
    companion object {
        ...
        class TestConfig {
            ...
            @Bean fun buffer(): Sinks.Many<PanelEvent<*, *>> = Sinks.many().unicast().onBackpressureBuffer()
            @Bean("panel-mock") fun panelEventQueueMock(buffer: Sinks.Many<PanelEvent<*, *>>, om: ObjectMapper): Supplier<Flux<String>> = Supplier {
        ...
        fun registerDynamicProperties(registry: DynamicPropertyRegistry) {
            ...
            registry.add("spring.cloud.function.definition") { "panel; panel-mock" }
            registry.add("spring.cloud.stream.bindings.panel-in-0.destination") { "panel-event" }
            registry.add("spring.cloud.stream.bindings.panel-mock-out-0.destination") { "panel-event" }
        }
        ...
        data class TestEvent (
            ...
            private val type: PanelEvent.Type
        ): PanelEvent<String, TestEvent> {
