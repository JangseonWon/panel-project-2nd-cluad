...
class AmlQc(
    ...
    om: ObjectMapper
): Aml, AbstractQcFileReader(analysisManager, mutex, sheet, om) {
    private val fileNameMatcher = "^(\\d{2}HEMA\\d{1,4})[-_]AML[-_]QC\\.csv$".toRegex()
    private val rowHeaderMatcher = "^((\\d{2}HEMA\\d{1,4})[-_](AML)[-_](\\d{1,4})[-_](\\d{8}-\\d{3}-\\d{4})).*".toRegex()
