...
class MmQc(
    ...
    mutex: Mutex,
    @Value("\${subscriber.mm.sheet}") val sheet: UUID,
    om: ObjectMapper
): Mm, AbstractQcFileReader(analysisManager, mutex, sheet, om) {
    private val fileNameMatcher = "^(\\d{2}HEMA\\d{1,4})[-_](MM|MMB|MMF)[-_]QC\\.csv$".toRegex()
    private val rowHeaderMatcher = "^((\\d{2}HEMA\\d{1,4})[-_](MM|MMB|MMF)[-_](\\d{1,4})[-_](\\d{8}-\\d{3}-\\d{4})).*".toRegex()
