package com.gcgenome.lims.analysis.actor.hema

import com.fasterxml.jackson.databind.ObjectMapper
import com.gcgenome.lims.analysis.*
import com.gcgenome.lims.analysis.actor.hema.v1.Hema
...
class HemaQc(
    ...
    om: ObjectMapper
): Hema, AbstractQcFileReader(analysisManager, mutex, sheet, om) {
    private val fileNameMatcher = "^(\\d{2}HEMA\\d{1,4})[-_]QC.csv".toRegex()
    private val rowHeaderMatcher = "^I_(\\d{2}HEMA\\d{1,4})[-_](.*)[-_](\\d{1,4})[-_](?:(\\d{8}-\\d{3}-\\d{4})|(?i)(positive))".toRegex()
