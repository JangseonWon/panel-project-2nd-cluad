...
class MdsSnv(
    ...
    mutex: Mutex
): Mds, AbstractSnvFileReader(analysisManager, snvManager, mutex, sheet) {
    override fun chkFormat(file: Path): Boolean = MdsFileFormatter.match(file.name) !=null
    override fun format(path: Path): SnvFileFormat? = MdsFileFormatter.match(path.name)
    override fun batch(path: Path): String? = MdsFileFormatter.match(path.name)?.batch
    object MdsFileFormatter {
        private val matcher = "^((\\d{2}HEMA\\d{1,4})[-_](MDS)[-_](\\d{1,4})[-_](\\d{8}-\\d{3}-\\d{4})).*".toRegex()
